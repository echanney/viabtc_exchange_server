...
# afterward guarantees that P2P talks to the newly compiled submission rather
# than to a process that was launched from an older executable.
...
# P2P must also compile the baseline repository. The supplied Dockerfile documents
# marketprice as a known base-commit source defect, so that one build remains
# best-effort here; every other component is mandatory.
build_all_components || { not_run_all "required server component build failed"; exit 1; }
if ! build_component marketprice make -C "$ROOT/marketprice" -j"$(nproc)"; then
  printf 'BUILD NOTE: marketprice is allowed to fail in P2P because the base commit is known not to compile it.\n'
fi

# Build the test client used to issue live HTTP, RPC, and CLI checks.
...
# Bring up the Dockerfile-provisioned infrastructure before matchengine starts.
# P2P may run after F2P in the same container, so every step is idempotent.
...
# Start a component from the executable compiled above and record its output.
start_component() {
  local dir="$1"
  local exe="$2"
  local log="$3"
...
# Wait for a TCP listener before sending the first black-box request.
wait_port() {
  local port="$1"
...
# Step 1: P2P preserves database state, but always starts fresh matchengine
# process from this build.
start_component matchengine matchengine.exe /tmp/golden_p2p_matchengine.log
if ! wait_port 7316; then
  echo 'FAIL: matchengine did not start; see /tmp/golden_p2p_matchengine.log' >&2
  cat /tmp/golden_p2p_matchengine.log >&2 || true
  not_run_all "matchengine startup failed"
  exit 1
fi
printf 'SERVICE PASS: matchengine RPC port is ready\n'

# Step 2: start and validate accesshttp before marketprice. This ordering is
# required by the existing marketprice implementation: init_market() performs an
# HTTP market.list request during startup.
start_component accesshttp accesshttp.exe /tmp/golden_p2p_accesshttp.log
if ! wait_port 8080 || ! wait_http_ready 80; then
  echo 'FAIL: accesshttp did not become HTTP-ready; see /tmp/golden_p2p_accesshttp.log' >&2
  cat /tmp/golden_p2p_accesshttp.log >&2 || true
  not_run_all "accesshttp HTTP readiness failed"
  exit 1
fi
printf 'SERVICE PASS: accesshttp returned a valid JSON response\n'

# Step 3: when marketprice built successfully, start it only after accesshttp is
# ready. The historical base commit may legitimately lack a marketprice executable
# or fail to build it, so P2P keeps this component best-effort.
if [[ -x "$ROOT/marketprice/marketprice.exe" ]]; then
  if ! start_marketprice_with_retry /tmp/golden_p2p_marketprice.log; then
    printf 'SERVICE NOTE: marketprice could not start in P2P; continuing because the base commit permits this defect.\n' >&2
  fi
fi

# Run the backward-compatibility integration suite against those fresh processes.
run_integration_suite p2p
